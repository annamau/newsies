...
from constants import  ABC_DATA_PATH, ABC_URL
    ...
    try:
        async with session.get(link, headers={'User-Agent':str(ua.random)}) as response:
            if response.status == 200:
                ...
                soup = bs(html, "lxml")
                paragrafs = soup.find_all("p", {"class": "voc-p"})
...
async def abc(limit=30):
    async with aiohttp.ClientSession() as session:
        html = requests.get(ABC_URL, headers={'User-Agent':str(ua.random)})
    ...
    with open(ABC_DATA_PATH, "w") as f:
        json.dump(data, f)

    print("\n All articles from ABC found and extracted! \n")
...
if __name__ == "__main__":
    ...
    print(f"  - Total time to collect from ABC: {end - start}s \n")